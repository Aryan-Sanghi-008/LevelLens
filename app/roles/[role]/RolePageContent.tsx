            ...
            <CardContent>
              <div className="space-y-4">
                {topCompanies.map((c) => (
                  <div
                    key={c.meta.slug}
                    onClick={() => setSelectedCompany(selectedCompany === c.meta.slug ? null : c.meta.slug)}
                    className={cn(
                      "flex items-center gap-3 p-1.5 rounded-lg hover:bg-muted/40 transition-colors cursor-pointer border border-transparent select-none",
                      selectedCompany === c.meta.slug && "border-brand-primary bg-brand-primary/5 shadow-xs"
                    )}
                  >
                    <CompanyLogo
                      src={c.meta!.logo}
